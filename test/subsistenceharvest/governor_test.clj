(ns subsistenceharvest.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [subsistenceharvest.store :as store]
            [subsistenceharvest.advisor :as advisor]
            [subsistenceharvest.governor :as governor]))
...
(defn- log-op [attached?]
  {:op :log-harvest-record :effect :propose :area-id "A-1"
   :harvest-report-attached? attached?
   :confidence 0.9 :stake :low})

(defn- supply-op [cost]
  {:op :coordinate-supply-order :effect :propose :area-id "A-1"
   :cost cost :confidence 0.9 :stake :low})

(def ^:private req {:operator-id "operator-1"})

;; -- routine documentation ops: ok/hard on area + harvest-report basis --
...
(deftest hard-on-unknown-area
  (let [st (fresh-store)
        v (governor/check req {} (assoc (log-op true) :area-id "A-ghost") st)]
    (is (:hard? v))
    (is (some #(= :unknown-area (:rule %)) (:violations v)))))

(deftest hard-on-area-wrong-operator
  (let [st (fresh-store)]
    (store/register-operator! st {:operator-id "operator-2" :name "Other"})
    (let [v (governor/check {:operator-id "operator-2"} {} (log-op true) st)]
      (is (:hard? v))
      (is (some #(= :area-wrong-operator (:rule %)) (:violations v))))))
...
(deftest hard-on-missing-area-id
  (let [st (fresh-store)
        v (governor/check req {} (dissoc (log-op true) :area-id) st)]
    (is (:hard? v))
    (is (some #(= :missing-area (:rule %)) (:violations v)))))
...
;; -- structural absence of weapon-firing/trap-setting/fishing-hunting-
;;    gathering-execution/kill-decision authority --
...
(deftest hard-on-op-outside-closed-allowlist
  (testing "no op resembling weapon-firing/trap-setting/fishing-hunting-gathering-execution/kill-decision exists in the allowlist"
    (doseq [phantom-op [:fire-weapon :discharge-weapon :set-trap :spring-trap
      ...
      (let [st (fresh-store)
        ...
        (is (or (some #(= :unknown-op (:rule %)) (:violations v))
                (some #(= :subsistence-execution-scope-excluded-op (:rule %)) (:violations v)))
            (str phantom-op " must trip :unknown-op or :subsistence-execution-scope-excluded-op"))))))

(deftest hard-on-scope-excluded-op-even-with-otherwise-valid-fields
  (testing "the scope-excluded-op check trips independently of the closed-allowlist check"
    (doseq [phantom-op [:fire-weapon :discharge-weapon :set-trap :spring-trap
                        :make-kill-decision :make-harvest-timing-decision
                        :make-fishing-execution-decision :make-hunting-execution-decision
                        :make-gathering-execution-decision :make-trapping-execution-decision]]
      (let [st (fresh-store)
            v (governor/check req {} (assoc (log-op true) :op phantom-op :confidence 0.99) st)]
        (is (some #(= :subsistence-execution-scope-excluded-op (:rule %)) (:violations v))
            (str phantom-op " must trip :subsistence-execution-scope-excluded-op"))))))

(deftest hard-on-subsistence-execution-scope-exclusion-text
  (testing "a proposal's free text cannot finalize/execute a weapon-firing/trap-setting/fishing-hunting-gathering-execution/kill-decision action even under a routine op"
    (doseq [phrase ["we will fire the weapon at the target"
                    ...
                    "propose we make the fishing execution decision now"
                    "recommend we make the hunting execution decision"
                    "advise we make the gathering execution decision"
                    "propose to make the trapping execution decision"]]
      (let [st (fresh-store)
            v (governor/check req {} (assoc (log-op true) :rationale phrase :confidence 0.99) st)]
        (is (:hard? v) (str "must hard-block: " phrase))
        (is (some #(= :subsistence-execution-scope-exclusion (:rule %)) (:violations v))
  ...
  (testing "bare nouns describing what was NOT done must not self-trip (only finalization/execution phrases do)"
    (let [st (fresh-store)
          v (governor/check req {}
                             (assoc (log-op true)
                                    :rationale "no weapon-firing, trap-setting/springing, fishing/hunting/gathering-execution or kill/harvest-timing decision proposed or implied")
                             st)]
      (is (not (some #(= :subsistence-execution-scope-exclusion (:rule %)) (:violations v)))))))
...
(deftest default-mock-advisor-proposals-never-self-trip-scope-exclusion
  (let [st (fresh-store)
        ...
        requests [{:op :log-harvest-record :operator-id "operator-1" :area-id "A-1"
                   :harvest-report-attached? true :stake :low}
                  {:op :schedule-equipment-operation :operator-id "operator-1" :area-id "A-1"
                   :stake :low}
                  {:op :flag-livelihood-concern :operator-id "operator-1" :area-id "A-1"
                   :stake :low}
                  {:op :coordinate-supply-order :operator-id "operator-1" :area-id "A-1"
                   :cost 500 :stake :low}]]
    (doseq [request requests]
      (let [proposal (advisor/-advise mock st request)
        ...
        (is (not (some #(= :subsistence-execution-scope-excluded-op (:rule %)) (:violations v)))
            (str "op " (:op request) " unexpectedly rejected by scope-excluded-op check"))))))

;; -- always-escalate ops / thresholds --

(deftest always-escalates-flag-livelihood-concern-even-at-high-confidence
  (testing "an observation that MAY warrant food-security/resource-depletion/safety review is NEVER auto-commit-eligible"
    (let [st (fresh-store)
          v (governor/check req {} {:op :flag-livelihood-concern :effect :propose
                                    :area-id "A-1" :confidence 0.99 :stake :low} st)]
      (is (not (:hard? v)))
      (is (:escalate? v))
      (is (not (:ok? v))))))
...
(deftest ok-supply-order-at-exact-threshold-boundary
  (testing "the supply-order cost threshold is inclusive"
    (let [st (fresh-store)
          v (governor/check req {} (supply-op 1000) st)]
      (is (:ok? v)))))
...
(deftest ok-schedule-equipment-operation
  (let [st (fresh-store)
        v (governor/check req {} {:op :schedule-equipment-operation :effect :propose
                                  :area-id "A-1" :confidence 0.9 :stake :low} st)]
    (is (:ok? v))))
